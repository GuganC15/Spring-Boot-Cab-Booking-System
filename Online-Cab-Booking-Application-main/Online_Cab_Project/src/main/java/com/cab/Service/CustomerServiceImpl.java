...
public class CustomerServiceImpl implements CustomerService{
	...
	public Customer insertCustomer(Customer customer) throws CustomerException {
		...
		if(cust.isPresent()) {
			...
			if(customer.getPassword().equalsIgnoreCase("Customer")) {	
				
				String tempval=customer.getPassword();
				
		        String emailRegex = "^[A-Z][A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z]{2,6}$";
		        Pattern pattern = Pattern.compile(emailRegex);
		        java.util.regex.Matcher matcher = pattern.matcher(tempval);
				...
				throw new CustomerException("please enter validate keyword kind of below : " + "> must be one use capital letter"   +" > must be one use number" );
			}
		}
		 else{
				throw new CustomerException("not a valid");
		}
